...
use systers::reporter::{generate_report, format_report};
...
fn print_usage() {
    println!("Systers Report - System Analysis Report Generator");
    println!();
    println!("USAGE:");
...
fn main() -> Result<()> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--hours" => {
                ...
                hours_back = args[i].parse()
                    .context("Invalid value for --hours")?;
            }
            _ => {
    ...
    // Generate report
    let (metrics, logs) = generate_report(&conn, hours_back)
