...
use crate::db::{LogEntry, query_metrics, query_logs};
...
pub fn generate_report(conn: &Connection, hours_back: i64) -> Result<(MetricsReport, LogReport)> {
    ...
    let metrics_report = if metrics.is_empty() {
        ...
        let avg_mem_pct = metrics.iter()
            .map(|m| (m.memory_used as f32 / m.memory_total as f32) * 100.0)
            .sum::<f32>() / count;
        let max_mem_pct = metrics.iter()
            .map(|m| (m.memory_used as f32 / m.memory_total as f32) * 100.0)
            .fold(0.0f32, f32::max);
        
        let avg_disk_pct = metrics.iter()
            .map(|m| (m.disk_used as f32 / m.disk_total as f32) * 100.0)
            .sum::<f32>() / count;
        let max_disk_pct = metrics.iter()
            .map(|m| (m.disk_used as f32 / m.disk_total as f32) * 100.0)
            .fold(0.0f32, f32::max);
        
        let avg_proc = (metrics.iter().map(|m| m.process_count).sum::<usize>() as f32 / count) as usize;
        let max_load = metrics.iter().map(|m| m.load_avg_1min).fold(0.0f64, f64::max);
        
        // Identify issues
        let mut issues = Vec::new();
        
        if max_cpu > 90.0 {
            issues.push(format!("⚠️  HIGH CPU USAGE: Peak CPU usage reached {:.1}%", max_cpu));
        }
        if max_mem_pct > 90.0 {
            issues.push(format!("⚠️  HIGH MEMORY USAGE: Peak memory usage reached {:.1}%", max_mem_pct));
        }
        if max_disk_pct > 85.0 {
            issues.push(format!("⚠️  HIGH DISK USAGE: Disk usage reached {:.1}%", max_disk_pct));
        }
        if max_load > 5.0 {
            issues.push(format!("⚠️  HIGH LOAD: System load average reached {:.2}", max_load));
        }
    ...
    let recent_errors: Vec<LogEntry> = all_logs.iter()
...
pub fn format_report(metrics: &MetricsReport, logs: &LogReport) -> String {
    ...
    output.push_str("╔════════════════════════════════════════════════════════════════╗\n");
    output.push_str("║              SYSTERS - SYSTEM ANALYSIS REPORT                  ║\n");
    output.push_str("╚════════════════════════════════════════════════════════════════╝\n\n");
    
    output.push_str(&format!("Report Period: {} to {}\n\n",
    ...
    output.push_str(&format!("Disk Usage:\n"));
    output.push_str(&format!("  Average: {:.1}%\n", metrics.avg_disk_used_percent));
    output.push_str(&format!("  Peak:    {:.1}%\n\n", metrics.max_disk_used_percent));
    
    output.push_str(&format!("System Load:\n"));
    output.push_str(&format!("  Peak (1-min avg): {:.2}\n\n", metrics.max_load_avg_1min));
    
    output.push_str(&format!("Average Process Count: {}\n\n", metrics.avg_process_count));
    ...
    if metrics.max_cpu_usage > 90.0 {
        recommendations.push("• Investigate high CPU usage - check for runaway processes");
    }
    if metrics.max_memory_used_percent > 90.0 {
        recommendations.push("• Memory usage is high - consider freeing up memory or adding more RAM");
    }
    if metrics.max_disk_used_percent > 85.0 {
